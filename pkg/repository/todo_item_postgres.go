...
func (r *TodoItemPostgres) Delete(userId, itemId int) error {
  query := fmt.Sprintf("DELETE FROM %s USING %s li, %s ul WHEREti.id = li.item_id AND li.list_id AND ul.user_id = $1 AND ti.id = $2",
                        todoItemsTable, listsItemsTable, usersListsTable)
...
func (r *TodoItemPostgres) Update(userId, itemId int, input todo.UpdateItemInput) error {
  ...
  query := fmt.Sprintf("UPDATE %s ti SET %s FROM %s FROM %s li, %s ul WHERE ti.id = li.item_id AND li.list_id = ul.list_id = ul.list_id AND ul.user_id = $%d AND ti.id = $%d",
